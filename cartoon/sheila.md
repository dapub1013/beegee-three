# Sheila's Arcane Trickster

[Build Reference](https://bg3.wiki/wiki/Arcane_Trickster) | [↓ Items](#items)

---

### Level 1

Class: Rogue

Expertise 2/2:
- Stealth
- Sleight of Hand

Ability Points

| Ability | Score | Modifier |
|---------|-------|----------|
| Strength | 8 | — |
| Dexterity | 17 | +2 |
| Constitution | 14 | — |
| Intelligence | 14 | +1 |
| Wisdom | 12 | — |
| Charisma | 10 | — |

---

### Level 2

Class: Rogue

---

### Level 3

Class: Rogue

Subclass: Arcane Trickster

Cantrips 2/2:
- Mage Hand
- Minor Illusion

Spells 3/3:
- Sleep
- Colour Spray
- Longstrider

---

### Level 4

Class: Rogue

Feat: Alert

---

### Level 5

Class: Rogue

Expertise 2/2:
- Perception
- Persuasion

---

### Level 6

Class: Rogue

Spells 1/1:
- Misty Step

---

### Level 7

Class: Rogue

Cantrips 1/1:
- Blade Ward

Spells 1/1:
- Hold Person

---

### Level 8

Class: Rogue

Feat: Ability Improvement +2 DEX

---

### Level 9

Class: Rogue

Spells 1/1:
- Haste

---

### Level 10

Class: Rogue

Spells 1/1:
- Greater Invisibility

---

### Level 11

Class: Rogue

Cantrips 1/1:
- Toll the Dead

Spells 1/1:
- Hold Monster

---

### Level 12

Class: Rogue

Feat: Ability Improvement +2 DEX

---

# Items

## Act 1

[ ] Armour: [Leather Armour +1](https://bg3.wiki/wiki/Leather_Armour_%2B1)
  - `Goblin Camp` Sold by **Grat the Trader**. `X: -99 Y: 424`

[ ] Boots: [Disintegrating Night Walkers](https://bg3.wiki/wiki/Disintegrating_Night_Walkers)
  - `Grymforge` Worn by **Nere**; looted from his corpse. `X: -854 Y: 780`
  - **Note:** Priority acquisition. Grants Misty Step and immunity to being enwebbed, entangled, or ensnared — essential for a character who needs to stay mobile and hidden.

[ ] Gloves: [Gloves of Dexterity](https://bg3.wiki/wiki/Gloves_of_dexterity)
  - `Crèche Y'llek` Sold by **A'jak'nir Jeera**. `X: 1380 Y: -798`
  - **Note:** Sets DEX to 18. Once equipped, redistribute ability points:

| Ability | Score | Modifier |
|---------|-------|----------|
| Strength | 8 | — |
| Dexterity | 8 | — |
| Constitution | 16 | +1 |
| Intelligence | 16 | +2 |
| Wisdom | 12 | — |
| Charisma | 10 | — |

[ ] Helmet: [Haste Helm](https://bg3.wiki/wiki/Haste_Helm)
  - `Blighted Village` Inside a Moss-Covered Chest near the well. `X: 29 Y: 405`

[ ] Ring: [Crusher's Ring](https://bg3.wiki/wiki/Crusher%27s_Ring)
  - `Goblin Camp` Worn by **Crusher**. Slip it off his toe when he demands you kiss his foot *or* loot from his corpse. `X: -70 Y: 439`

[ ] Ring: [Ring of Protection](https://bg3.wiki/wiki/Ring_of_Protection)
  - `Tiefling Hideout` Rewarded by **Mol** for completing the quest *Steal the Sacred Idol* in the Emerald Grove. `X: 84 Y: 73`

[ ] Shortsword: [Shortsword of First Blood](https://bg3.wiki/wiki/Shortsword_of_First_Blood)
  - `Underdark` Looted from an executed Deep Gnome corpse near the Myconid Colony entrance. `X: 73 Y: -187`

[ ] Shortsword: [Knife of the Undermountain King](https://bg3.wiki/wiki/Knife_of_the_Undermountain_King)
  - `Crèche Y'llek` Sold by **A'jak'nir Jeera**. `X: 1380 Y: -798`

## Act 2

[ ] Armour: [Yuan-Ti Scale Mail](https://bg3.wiki/wiki/Yuan-Ti_Scale_Mail)
  - `Last Light Inn` Sold by **Quartermaster Talli** near the Last Light Inn waypoint. `X: -31 Y: 130`

[ ] Cloak: [Cloak of Cunning Brume](https://bg3.wiki/wiki/Cloak_of_Cunning_Brume)
  - `Last Light Inn` Sold by **Mattis**. `X: -56 Y: 141`

[ ] Helmet: [Covert Cowl](https://bg3.wiki/wiki/Covert_Cowl)
  - `Last Light Inn` Looted from a **Meenlock** in the cellar. `X: 33 Y: -697`

[ ] Ring: [Eversight Ring](https://bg3.wiki/wiki/Eversight_Ring)
  - `House of Healing` Found in a locked opulent chest in the corner of the morgue lab west of the zombie crypt. `X: 9 Y: -981`

[ ] Ring: [Shadow-Cloaked Ring](https://bg3.wiki/wiki/Shadow-Cloaked_Ring)
  - `Shadow-Cursed Lands` Looted from the **Shadow Mastiff Alpha**. `X: -49 Y: 36`

[ ] Shortsword: [Sword of Life Stealing](https://bg3.wiki/wiki/Sword_of_Life_Stealing)
  - `Last Light Inn` Sold by **Dammon**. `X: -33 Y: 164`

## Act 3

[ ] Armour: [Armour of Agility](https://bg3.wiki/wiki/Armour_of_Agility)
  - `Lower City` Sold by **Gloomy Fentonson** at Stormshore Armoury. `X: -41 Y: -71`

[ ] Boots: [Helldusk Boots](https://bg3.wiki/wiki/Helldusk_Boots)
  - `Wyrm's Rock Fortress` Found in a locked Gilded Chest on the top floor. `X: -32 Y: 219`

[ ] Cloak: [Shade-Slayer Cloak](https://bg3.wiki/wiki/Shade-Slayer_Cloak)
  - `Lower City` Sold by **Sticky Dondo** in the Guildhall. `X: -17 Y: 755`

[ ] Dagger: [Bloodthirst](https://bg3.wiki/wiki/Bloodthirst)
  - `Bhaal Temple` Carried by **Orin** during the quest *Get Orin's Netherstone*. `X: 61 Y: 1004`

[ ] Gloves: [Helldusk Gloves](https://bg3.wiki/wiki/Helldusk_Gloves)
  - `House of Hope` Worn by **Haarlep** in the Boudoir. `X: -6478 Y: 2993`

[ ] Helmet: [Mask of Soul Perception](https://bg3.wiki/wiki/Mask_of_Soul_Perception)
  - `Devil's Fee` In a locked Gilded Chest (DC 20 Sleight of Hand) in **Helsik's** room upstairs. `X: -33 Y: 20`

[ ] Shortsword: [Knife of the Undermountain King](https://bg3.wiki/wiki/Knife_of_the_Undermountain_King)
  - `Crèche Y'llek` Sold by **A'jak'nir Jeera**. `X: 1380 Y: -798`
  - **Note:** Off-hand weapon for the full playthrough.

---

# Notes

**Mage Hand Legerdemain:** Arcane Trickster's version of Mage Hand can pick locks, disarm traps, and steal items from a distance without Sheila being in range. This is the build's defining utility feature — use it before every chest and trap encounter rather than walking Sheila into danger.

**Invisibility cloak fantasy:** Sheila's cloak is her signature. Shade-Slayer Cloak (Act 3) makes her invisible when she enters shadows. Cloak of Cunning Brume (Act 2) creates a smoke cloud on Disengage. Shadow-Cloaked Ring grants Sneak Attack while in dim light or darkness. The build leans into all of these — position her in shadow, attack from concealment, vanish after.

**Sneak Attack:** At level 12 Sneak Attack deals 6d6 bonus damage on qualifying hits. Sheila needs advantage or an adjacent ally to proc it. The Shadow-Cloaked Ring removes the adjacency requirement in dim light. Greater Invisibility (level 10 spell) grants advantage on all attacks while invisible — combine with a high-damage strike to delete a priority target.

**Alert feat:** Like Presto, Sheila dies if she acts after enemies spot her. Alert ensures she gets Stealth and positioning before the fight starts properly. Invisibility on round one is only useful if she gets to cast it before anyone locks onto her.

**Gloves of Dexterity transition:** Grab these at Crèche Y'llek and immediately reallocate — dump the points that were in DEX into CON and INT. Sheila's spell save DC and Arcane Trickster spell attacks scale with INT, so the reallocation genuinely improves her control spells.

**Hold Person into Sneak Attack:** Held targets are paralyzed, and melee attacks against paralyzed creatures automatically crit. Sheila cannot deliver the melee attack herself reliably, but she can hold a target and let Diana or Bobby walk up for a guaranteed critical. Coordinate this in fights against named humanoid enemies.
