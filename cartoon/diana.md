# Diana's Flawless Acrobat

[Build Reference](https://www.youtube.com/watch?v=waCUspr6EUU) | [↓ Items](#items)

---

### Level 1

Class: Monk

Ability Points

| Ability | Score | Modifier |
|---------|-------|----------|
| Strength | 16 | +2 |
| Dexterity | 14 | — |
| Constitution | 16 | +1 |
| Intelligence | 8 | — |
| Wisdom | 14 | - |
| Charisma | 8 | — |

---

### Level 2

Class: Monk

---

### Level 3

Class: Monk

Subclass: Way of the Open Hand

---

### Level 4

Class: Monk

Feat: Tavern Brawler

  - **Note:** Unequip main hand weapon once taking Tavern Brawler. Unarmed strikes now scale with STR and apply the Tavern Brawler bonus.

---

### Level 5

Class: Monk

---

### Level 6

Class: Monk

---

### Level 7

Class: Rogue

---

### Level 8

Class: Rogue

Subclass: Thief

---

### Level 9: **RESPEC**

Redistribute as follows. Take levels 1–6 as above, then continue from Level 7.

### Level 1

Class: Monk

Ability Points

| Ability | Score | Modifier |
|---------|-------|----------|
| Strength | 16 | +2 |
| Dexterity | 14 | — |
| Constitution | 16 | +1 |
| Intelligence | 8 | — |
| Wisdom | 14 | - |
| Charisma | 8 | — |

### Level 2

Class: Monk

### Level 3

Class: Monk

Subclass: Way of the Open Hand

### Level 4

Class: Monk

Feat: Tavern Brawler

  - **Note:** Unequip main hand weapon once taking Tavern Brawler.

### Level 5

Class: Monk

### Level 6

Class: Monk

### Level 7

Class: Rogue

### Level 8

Class: Rogue

Subclass: Thief

### Level 9

Class: Rogue

### Level 10

Class: Monk

Feat: Ability Improvement +2 STR

### Level 11

Class: Monk

### Level 12

Class: Monk

Feat: Ability Improvement +2 WIS

---

# Items

## Act 1

[ ] Amulet: [Moondrop Pendant](https://bg3.wiki/wiki/Moondrop_Pendant)
  - `Owlbear Nest` Found in a Selûnite Gilded Chest; read the **Selunite Prayer Sheet** in front of the chest to unlock it. `X: -321 Y: -135`

[ ] Armour: [Bloodguzzler Garb](https://bg3.wiki/wiki/Bloodguzzler_Garb)
  - `Underdark` Looted from the **Bulette**.

[-] Boots: [Boots of Striding](https://bg3.wiki/wiki/Boots_of_Striding)
  - `Shattered Sanctum` Worn by **Minthara**. `X: 335 Y: 43`

[ ] Boots: [Boots of Genial Striding](https://bg3.wiki/wiki/Boots_of_Genial_Striding)
  - `Ebonlake Grotto` Sold by **Blurg**. `X: 111 Y: -89`
  - **Note:** Difficult terrain immunity keeps Diana in melee range without burning
    movement. Strong value for a monk crossing Grease, Spike Growth, and Web zones.
    No armour proficiency required.

[-] Gloves: [Gloves of Dexterity](https://bg3.wiki/wiki/Gloves_of_dexterity)
  - `Crèche Y'llek` Sold by **A'jak'nir Jeera**. `X: 1380 Y: -798`

[-] Helmet: [Haste Helm](https://bg3.wiki/wiki/Haste_Helm)
  - `Blighted Village` Inside a Moss-Covered Chest near the well. `X: 29 Y: 405`

[ ] Helmet: [Helmet of Uninhibited Kushigo](https://bg3.wiki/wiki/Helmet_of_Uninhibited_Kushigo)
  - `Underdark` On the floor near the northern edge of the Myconid Colony. `X: 85 Y: 6`
  - **Note:** Deals 1d4 bonus damage on unarmed strikes while not wearing armour.
    Directly amplifies the Tavern Brawler unarmed loop. Verify coordinates in-game —
    Underdark item placement can shift across patches.

[ ] Ring: [Crusher's Ring](https://bg3.wiki/wiki/Crusher%27s_Ring)
  - `Goblin Camp` Worn by **Crusher**. Slip it off his toe when he demands you kiss his foot *or* loot from his corpse. `X: -70 Y: 439`

[-] Ring: [Ring of Protection](https://bg3.wiki/wiki/Ring_of_Protection)
  - `Tiefling Hideout` Rewarded by **Mol** for completing the quest *Steal the Sacred Idol* in the Emerald Grove. `X: 84 Y: 73`

[ ] Shortbow: [Hunting Shortbow](https://bg3.wiki/wiki/Hunting_Shortbow)
  - `The Hollow` Sold by **Dammon** in the Emerald Grove. `X: 177 Y: 562`

## Act 2

[-] Cloak: [Cloak of Protection](https://bg3.wiki/wiki/Cloak_of_Protection)
  - `Last Light Inn` Sold by **Quartermaster Talli** near the Last Light Inn waypoint. `X: -31 Y: 130`

> ⚠️ **No Act 2 cloak alternative confirmed for Diana.** She carries an empty cloak
> slot through Act 2. Talli relocates to Moonrise Towers before the end of Act 2 and
> may have a second copy of the Cloak of Protection in stock there — purchase it for
> Diana if available. Otherwise Diana's cloak slot remains open until Act 3.

[-] Ring: [Callous Glow Ring](https://bg3.wiki/wiki/Callous_Glow_Ring)
  - `Gauntlet of Shar` In an opulent chest in the vault room near **Balthazar**. `X: -821 Y: -752`

[ ] Shortbow: [Darkfire Shortbow](https://bg3.wiki/wiki/Darkfire_Shortbow)
  - `Last Light Inn` Sold by **Dammon** in Act 2. `X: -33 Y: 164`

## Act 3

[ ] Amulet: [Khalid's Gift](https://bg3.wiki/wiki/Khalid%27s_Gift)
  - `Elerrathin's Home` In a display case in the basement. `X: -1572 Y: 976`
  - **Note:** May not be available if Jaheira is not recruited.

[ ] Armour: [Vest of Soul Rejuvenation](https://bg3.wiki/wiki/Vest_of_Soul_Rejuvenation)
  - `Sorcerous Sundries` Sold by **Lorroakan's Projection** if Rolan is dead.
  - `Sorcerous Sundries` Sold by **Rolan** if Rolan and Lorroakan are both alive.
  - `Ramazith's Tower` Sold by **Rolan** via trade option on the bottom left during dialogue if Rolan is alive but Lorroakan is dead.

[ ] Boots: [Boots of Uninhibited Kushigo](https://bg3.wiki/wiki/Boots_of_Uninhibited_Kushigo)
  - `Astral Plane` Carried by **Prelate Lir'i'c** as entering Act 3. `X: -163 Y: -1158`

[ ] Gloves: [Gloves of Soul Catching](https://bg3.wiki/wiki/Gloves_of_Soul_Catching)
  - `House of Hope` Given by **Hope** for completing her quest *Save Hope*. `X: -6477 Y: 2884`
  - **Note:** Once equipped alongside Elixirs of Cloud Giant Strength, redistribute ability points:

| Ability | Score | Modifier |
|---------|-------|----------|
| Strength | 8 | — |
| Dexterity | 16 | +2 |
| Constitution | 15 | +1 |
| Intelligence | 8 | — |
| Wisdom | 20 | - |
| Charisma | 8 | — |

[ ] Helmet: [Horns of the Berserker](https://bg3.wiki/wiki/Horns_of_the_Berserker)
  - `Danthelon's Dancing Axe` Sold by **Entharl Danthelon**. `X: -10 Y: 143`

---

# Elixirs

[Elixirs of Hill Giant Strength](https://bg3.wiki/wiki/Elixir_of_Hill_Giant_Strength)
- Buy from **Auntie Ethel** *before* starting her quest. `Druid's Grove`
- **Note:** Use until Elixirs of Cloud Giant Strength are available. Do not go south of the `Blighted Village` until these are no longer needed.

[Elixirs of Cloud Giant Strength](https://bg3.wiki/wiki/Elixir_of_Cloud_Giant_Strength)
- Use once available in Act 3 alongside Gloves of Soul Catching.

---

# Notes

**Tavern Brawler:** The entire build pivots on this feat. It adds STR modifier to unarmed attack and damage rolls on top of the normal bonus, effectively doubling the STR contribution. With STR 18 via ASI and Elixirs of Hill Giant Strength (STR 21), unarmed strikes hit well above their weight class. Unequip any main hand weapon after taking the feat — Diana fights with her hands.

**Open Hand on Flurry of Blows:** Every Flurry proc can knock prone, push 5 feet, or prevent reactions. On tactician this is critical crowd control — a prone enemy grants advantage to all melee attackers and wastes their turn standing up. Combine with Stunning Strike (spend ki to force a CON save or stun until the start of their next turn) for hard single-target lockdown.

**Two bonus actions post-respec:** At level 9 (post-respec), Thief's Fast Hands provides a second bonus action. Diana can Flurry of Blows (bonus action 1) and still use Cunning Action to Dash or Disengage (bonus action 2), or spend both on ki-powered abilities. This is what separates the respec build from the pre-respec version.

**Gloves of Soul Catching transition:** These gloves restore HP on unarmed kills and add force damage to unarmed strikes. Once equipped with Cloud Giant Strength elixirs (STR 27), drop Gloves of Dexterity and redistribute. WIS 20 also pushes ki save DC and unarmored AC (WIS + DEX) to their ceiling.

**Unarmored Movement:** By level 9 Diana has +20 feet of movement on top of base speed. She should be the most mobile character in the party — leap into a group, Flurry, knock prone, leap out.
